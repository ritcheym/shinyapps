...
shinyServer(function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    dtarget <- dnorm(target,mean=input$dprime,sd=1) # sd=1 for equal variance SD
    ...
    par(new=T)
    # add recollection bar
    barplot(input$recollection,space=9,ylim=c(-.04,1),xlim=c(0,10),axes=F,xlab="",ylab="")
    axis(4)
    mtext("Recollection",side=4,line=3)
    par(new=F)
  },height=400,width=400)
  
  output$rocPlot <- renderPlot({
    # get response probabilities for each distribution
    plure <- 1 - pnorm(lure,mean=0,sd=1) # assuming no false recollection - otherwise multiply by (1-Rn)
    ptarget <- input$recollection + (1-input$recollection)*(1-pnorm(target,mean=input$dprime,sd=1))
    
    # get response probabilities at criterion
    plure.crit <- 1 - pnorm(input$criterion,mean=0,sd=1)
    ptarget.crit <- input$recollection + (1-input$recollection)*(1-pnorm(input$criterion,mean=input$dprime,sd=1))
